...
Steward facts are extracted only from JSON files whose top-level property is an **array of objects**
(each fact keyed by a field you name in `.true-up.json`). A code file (e.g. a `.py` registry) is **not**
auto-decomposed into facts, but it is a valid endpoint for a declared **seed** edge, so a coarse
file-level dependency on it is still tracked. If you declare no facts and no seed edges, the build
prints a NOTICE: the drift layer is inert (it passes `--check` trivially) until you declare something.
